...
function showAccessInfo() {
  ...
  console.log(`Adminer:       ${YELLOW('http://localhost:8083')} (MSSQL GUI)`);
  ...
  console.log(`MSSQL:         ${YELLOW('localhost:1433')} (sa / StrongP@ssw0rd)`);
  console.log();
}
...
function startServices(type) {
  switch (type) {
    ...
    case 'db':
      ...
      shell.exec('docker-compose up -d mysql postgres mssql phpmyadmin pgadmin adminer');
      showAccessInfo();
      break;
    case 'backend':
...
async function showLogs() {
  ...
  inquirer.prompt([
    ...
    const cmd = {
      ...
      'MSSQL': 'docker-compose logs mssql'
    }[service];
    shell.exec(cmd);
  });
}
...
function checkDbHealth() {
  ...
  const checks = [
    ...
    {
      name: 'MSSQL',
      cmd: 'docker-compose exec mssql /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "StrongP@ssw0rd" -Q "SELECT 1"',
    }
  ];
